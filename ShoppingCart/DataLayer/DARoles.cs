...
namespace DataLayer
{
    public class DARoles : ConnectionClass
    {
        ...
        /// <param name="UserId"></param>
        /// <returns></returns>
        public IQueryable<CommonLayer.Role> GetUserRoles(string Email)
        {
            return (from UserAccount in this.Entities.UserAccounts
                    from User in this.Entities.Users
                    from Role in UserAccount.Roles
                    where UserAccount.ID == User.UserAccountID && User.Email == Email
                    select Role);
        }
